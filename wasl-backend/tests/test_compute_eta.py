...
from app.tools.compute_eta import _count_holiday_days_between, compute_eta
...
class TestBasicSLA:
    ...
    def test_breached_when_deadline_passed(self):
        now = datetime(2026, 4, 10, 0, 0, tzinfo=UTC)
        s = _with_deadline(datetime(2026, 4, 8, 0, 0, tzinfo=UTC))
        r = compute_eta(s, now=now)
...
class TestHolidayLogic:
    ...
    def test_counts_eid_al_fitr_days(self):
        start = datetime(2026, 3, 18, 16, 0, tzinfo=UTC)
        end = datetime(2026, 3, 24, 18, 0, tzinfo=UTC)
    ...
    def test_holiday_delay_not_flagged_as_urgent(self):
        now = datetime(2026, 3, 18, 16, 0, tzinfo=UTC)
        deadline = datetime(2026, 3, 24, 18, 0, tzinfo=UTC)
